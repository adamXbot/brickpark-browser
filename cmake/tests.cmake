# cmake/tests.cmake -- owned by scope PORT-C (docs/SCOPE_PORT_WAVE.md).
...
# decoders in decomp/tools (tools/oracle_*.py). Notes, the result table and every
...
#
#   cmake -S . -B build -G Ninja -DPython3_EXECUTABLE=$PY
#   ninja -C build legoland_tests && ctest --test-dir build
#
#   emcmake cmake -S . -B build-wasm -G Ninja -DLL_ILP32=ON \
#       -DPython3_EXECUTABLE=$PY
#   ninja -C build-wasm legoland_tests      # waits on PORT-A's closure
...
# from the decomp/LEGOLAND/*.c ones). The guard makes it disappear by itself the
...
function(ll_oracle name)
  add_custom_command(
    ...
    COMMAND "${CMAKE_COMMAND}" -E env "LL_GAMEDATA=${LL_ROOT}/gamedata"
            "${Python3_EXECUTABLE}" "${LL_DECOMP}/tools/oracle_${name}.py"
            --header "${LL_ORACLE_DIR}/oracle_${name}.h" ${ARGN}
    DEPENDS "${LL_DECOMP}/tools/oracle_${name}.py"
...
"/* GENERATED by cmake/tests.cmake: gamedata/ is absent, so
...
#   ninja -C build legoland_dmusic
#   build/legoland_dmusic render --dir gamedata/main --dls build/dls/gm.dls \
...
if(LL_HAVE_GAMEDATA)
  add_test(NAME comp_format8
           COMMAND "${Python3_EXECUTABLE}" "${LL_DECOMP}/tools/comp.py"
...
# ---- the player page's data libraries (src/web/js) -----------------
