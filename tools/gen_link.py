...
CRT_PROTOS = {
    ...
    # src/hostwin/msvcrt.c, so `crt_proto` only reaches them if that
...
def check_rawwords(build_dir, baseline=None):
    ...
    that is not in `tests/rawwords_baseline.txt`, or a row whose word
...
def main():
    ...
    ap.add_argument('--baseline', help='the accepted raw-word residue '
                    '(default tests/rawwords_baseline.txt)')
    args = ap.parse_args()
    ...
    if variadic_bad:
        ...
        print(f'gen_link: REFUSING to close the link -- '
              ...
              f'(see above, and tools/variadic_sweep.py). Nothing was '
    ...
    # ---- the names the sources give TWO addresses (PORT-P3 §3.3, PORT-A10) --
    # `lr.scan_sources` keeps one address per name, with a `setdefault`: the
    # first file wins and nothing downstream is told there was a second. That
    # silent choice is how the scroll clamp came to read the coaster's zeroed
    # clip rect and how `g_popup` came to be sheared by 0x1c, which between them
    # cost the map its eastern 475 px and the game its ability to hire ANY
    # worker. The generator cannot fix it -- one of the two spellings needs a
    # different name, and LEGOLAND/*.c is not its to edit -- but it must not
    # hide it: every collision goes in the manifest and into globals.c itself,
    # with the address the generator actually emitted marked.
    # `tools/addr_sweep.py` is the gate; this is the notice.
    addr_collisions = addr_sweep.name_collisions()
    globals_c = ['/* generated by tools/gen_link.py: extern-only globals rebuilt from legoland.exe */', '']
    if addr_collisions:
        globals_c += [
            '/* ==========================================================================',
            f' * {len(addr_collisions)} NAME(S) ARE DECLARED AT MORE THAN ONE ADDRESS.',
            ' *',
            ' * One storage object is planned per NAME, so each row below collapsed two',
            ' * objects into one and every translation unit that spelled the OTHER address',
            ' * is reading and writing the wrong memory -- shifted by the difference, with',
            ' * no diagnostic anywhere. On x86 this was legal: each .obj carried its own',
            ' * address and the real linker gave each spelling the object it meant. The',
            ' * bytes VC6 emits are identical either way, so audit.py, relocs.py, match.py',
            ' * and verify.py cannot see this and never will.',
            ' *',
            ' * `->` marks the address this file actually DEFINES the name at. Every other',
            ' * address on the row is a spelling that silently lost.',
            ' *',
            ' * The fix is always in the game sources: one of the two spellings is a',
            ' * different object and needs a different name. See',
            ' * docs/lanes/scope-port-p3.md §3 and docs/lanes/scope-port-a10.md §1; the',
            ' * ruling for each row is tests/addr_collisions.txt.',
            ' */']
        for name, by, rename in addr_collisions:
            picked = externs.get(name, (None, None))[0]
            globals_c.append(f'/*   {name}')
            for a in sorted(by):
                mark = '->' if a == picked else '  '
                where = ', '.join(sorted({c.where for c in by[a]}))
                globals_c.append(f' *   {mark} 0x{a:08x}  {where}')
            if rename:
                globals_c.append(f' *      (mitigated: {rename[1]}:{rename[2]} '
                                 f'has #define {name} {rename[0]})')
            globals_c.append(' */')
        globals_c += [
            '/* Turn the notice above into a hard stop with',
            ' * -DLL_FAIL_ON_ADDR_COLLISION: useful once PORT-M15 has renamed them, to',
            ' * keep a new one from being merged. Off by default, because the build has',
            ' * to keep running while the known rows are open. */',
            '#ifdef LL_FAIL_ON_ADDR_COLLISION',
            f'#error "gen_link: {len(addr_collisions)} name(s) declared at two '
            f'addresses -- see the block above and tests/addr_collisions.txt"',
            '#endif',
            '']
    manifest = ['# gen_link manifest', '']
    aliases_c = ['/* generated by tools/gen_link.py: symbol forwarding */', '']
    stubs_c = ['/* generated by tools/gen_link.py: unwritten game functions */',
               ...
               '    exit(70);',
               '}', '']
    host_c = ['/* generated by tools/gen_link.py: host API the shim does not implement yet */',
              '#include "ll_gen.h"', '']
    ...
    manifest += [
        ...
        f'- names with conflicting addresses: {len(addr_collisions)}'
        + (f' ({", ".join(n for n, _b, _r in addr_collisions)})'
           if addr_collisions else ''),
        f'- pointer re-pointing (ilp32): {"on" if args.ilp32 else "off"}',
    ]
    if addr_collisions:
        manifest += [
            '', '## Names declared at more than one address', '',
            'One name, two addresses, in two different files. Only ONE storage',
            'object is planned per name, so every TU that spelled the other',
            'address is sheared by the difference with no diagnostic. On x86',
            'each .obj carried its own address and both spellings resolved to',
            'the object they meant, so the shipped image is correct and every',
            'byte gate the project has (audit.py, relocs.py, match.py,',
            'verify.py) is blind to this state AND to the fixed one.',
            '',
            'PORT-P3 measured two of these as blockers in the running tab: the',
            'map scroll clamp reads the coaster\'s zeroed 3D clip rect instead',
            'of its own 243200-unit slack (475 px of the map unreachable), and',
            '`g_popup` is sheared by 0x1c so no gardener and no mechanic can be',
            'hired anywhere in the game.',
            '',
            '`->` is the address this generator emitted. The fix belongs in the',
            'game sources -- one spelling is a different object and needs a',
            'different name. The rulings are in',
            '`tests/addr_collisions.txt`, gated by the `addr_sweep`',
            'ctest.', '',
            '| name | addresses | sites |', '| --- | --- | --- |']
        for name, by, rename in addr_collisions:
            picked = externs.get(name, (None, None))[0]
            addrs = ' '.join(('**' + f'0x{a:08x}' + '**' if a == picked
                              else f'0x{a:08x}') for a in sorted(by))
            sites = ', '.join(
                f'`{c.where}` 0x{a:08x}'
                for a in sorted(by)
                for c in sorted(by[a], key=lambda c: (c.file, c.line)))
            note = (f' (mitigated by `#define {name} {rename[0]}` at '
                    f'{rename[1]}:{rename[2]})' if rename else '')
            manifest.append(f'| `{name}` | {addrs} | {sites}{note} |')
        print(f'gen_link: WARNING {len(addr_collisions)} name(s) declared at '
              f'more than one address -- one object emitted for each, every '
              f'other spelling silently sheared. See the manifest and '
              f'tests/addr_collisions.txt.')
    if interiors:
    ...
    ext_md = ['# Source-derived object extents', '',
              "Computed by `tools/cdecl.py` from the game's own",
    ...
    if rw_rows:
        ...
        rw_md += ['', f'{len(rw_rows)} objects, {n_rw_unvisited} words.', '',
                  ...
                  'The baseline `tests/rawwords_baseline.txt` is this list as',
